...
{
    /// <summary>Open the app's Handoffs directory in the OS file manager (desktop / editor).</summary>
    public static class PcHandoffsFolderOpener
    {
        ...
        public static void TryOpenHandoffsFolder()
        {
            var handoffs = Path.Combine(UnityEngine.Application.persistentDataPath, "Handoffs");
            Directory.CreateDirectory(handoffs);
            var full = Path.GetFullPath(handoffs);
            ...
            {
                UnityEngine.Debug.LogWarning($"Could not open Handoffs folder: {ex.Message}\n{full}");
            }
        }
    }
}
