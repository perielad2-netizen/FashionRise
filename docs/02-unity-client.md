...
| **`Presentation/`** | Screens/flows: onboarding, login/register, home, create flow, template picker, fabric/material picker, color palette, model preview placeholder, gallery/profile, export. |
...
| Login / session restore | `Presentation/` + `IAuthService` (mock vs API) |
...
- **Composition:** `FashionRiseApp` + `AppServices` (HTTP API **by default** via `ApiConfig`; mocks optional in inspector) + `ScreenController` + `NavigationService`.
- **Navigation payloads:** `DesignDetailNavContext` (`GalleryItemId`); `GalleryNavContext` (**`OwnerUserId`** for per-creator public list, optional **`CommunitySort`** when `OwnerUserId` is empty — e.g. home **Gallery** / **Following feed** / post-publish newest).
